// components/disciplinas/futbol/femenino/FemeninoUI.jsx
"use client";

import Image from "next/image";
import { useMemo } from "react";

/* Reusamos UI de Masculino para mantener diseño idéntico */
export {
...
function IconCalendar(props) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
      <path d="M7 2a1 1 0 0 1 1 1v1h8V3a1 1 0 1 1 2 0v1h1.5A2.5 2.5 0 0 1 22 6.5v13A2.5 2.5 0 0 1 19.5 22h-15A2.5 2.5 0 0 1 2 19.5v-13A2.5 2.5 0 0 1 4.5 4H6V3a1 1 0 0 1 1-1Zm12.5 8h-15a.5.5 0 0 0-.5.5v9a.5.5 0 0 0 .5.5h15a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5ZM4 8h16V6.5a.5.5 0 0 0-.5-.5H18v1a1 1 0 1 1-2 0V6H8v1a1 1 0 1 1-2 0V6H4.5a.5.5 0 0 0-.5.5V8Z" />
    </svg>
  );
}

/* ========= banner “Sumate” (compacto, sin pills) ========= */

export function FlyerSumateFemeninoFull({ data, backgroundImageUrl }) {
  const d = data || {};
  ...
  return (
    <section className="relative left-1/2 right-1/2 w-screen -translate-x-1/2 overflow-hidden border-y border-red-800/30 bg-black">
      ...
      {/* Overlays (estáticos, más livianos que el gradiente animado) */}
      ...
      <div className="relative z-30 mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-10">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center gap-3 md:gap-6 py-3 sm:py-4">
          ...
          <div className="min-w-0 flex flex-col items-center md:items-end gap-2">
            ...
            {/* Nota corta opcional */}
            {d.note ? (
              <p className="text-[11px] text-white/70 text-center md:text-right max-w-[520px]">
                {d.note}
              </p>
            ) : null}
          </div>
...
/* =========================================================
   PETIT TORNEO (SEMIS IDA/VUELTA + FINAL ÚNICA)
   - Misma estructura visual que TablaLlavesCard (repechaje)
   - Semis: usa filas de "series" (ida/vuelta)
   - Final: usa fila de partido único (score), con fallback resultado||ida
   ========================================================= */
...
function SeriesScorePill({ children }) {
  // idéntico a MasculinoUI (ida/vuelta)
  return (
...
function ScorePill({ children }) {
  // idéntico a MasculinoUI (partido único)
  return (
...
export function TablaPetitLikeRepechaje({
  rows,
  title = "Petit torneo",
  phase = "Semis ida/vuelta + Final",
  footnote,
  useShortNames = true,
}) {
  const groups = useMemo(() => {
    ...
    const semiStages = [...byStage.keys()]
      .filter((k) => isSemiStage(k))
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-4 text-gray-900 min-w-0">
      ...
      {/* EXACTAMENTE la misma grilla/estilos que TablaLlavesCard */}
              ...
              <div className="divide-y">
                {(groups.final || []).length ? (
                  groups.final.map((t, i) => {
                    const res = String(t?.resultado || "").trim();
                    const ida = String(t?.ida || "").trim();

                    // ✅ fix partido único: si resultado no viene, usamos ida
                    const score =
                      res && res !== "-" && res !== "—"
                        ? res
                        : ida && ida !== "-" && ida !== "—"
