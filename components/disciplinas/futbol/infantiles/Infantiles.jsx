...
} from "./infantiles.config";

import {
  FlyerSumateInfantilesFull,
  CategoryPillsCard,
  TablaPosicionesInfantilesLike,
...
export default function Infantiles({ nav, active, onChange }) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const spString = searchParams.toString();

  const defaultCategoryId = "cat_a";
  ...
  return (
    <FutbolLayout hero={HERO} nav={nav} active={active} onChange={onChange} contact={CONTACT} quickFacts={QUICK_FACTS}>
      ...
      <FlyerSumateInfantilesFull data={JOIN_AD} backgroundImageUrl={BACKGROUND_IMAGE_URL} />
