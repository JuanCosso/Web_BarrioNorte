...
/* ========= flyer (idéntico lenguaje que femenino) ========= */

export function FlyerSumateInferioresFull({ data, backgroundImageUrl }) {
  const d = data || {};
      ...
      <div className="relative z-30 mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-10">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center gap-3 md:gap-6 py-3 sm:py-4">
          ...
          <div className="min-w-0 text-center md:text-left">
            ...
            {d.secondary ? (
              <p className="mt-1 text-[11px] sm:text-xs text-white/70">{d.secondary}</p>
            ) : null}
          </div>
...
export function TablaPosicionesInferioresLike({
  title,
  phase,
  equipos,
  footnote,
  withPositionColors = true, // en inferiores lo querés siempre
}) {
...
function SeriesScorePill({ children }) {
  // compacto: números chicos como en repechaje
  return (
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0 flex items-center gap-2">
        ...
        {/* IMPORTANTE: truncate para que NO apile letras */}
        <span
          className={[
...
export function TablaFinalesIdaVueltaCard({
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-4 text-gray-900 min-w-0">
      ...
      {/* CLAVE: items-start (no estira altura por la tabla de liga) */}
